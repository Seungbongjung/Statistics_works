...
post_ratio=function(m1,m2,h1,h2,n,samp,edge1,edge2,v,lambda,q){
  ...
  result=(q/((1-q)*v*sqrt(2*pi))*sqrt(4*pi/n))^edge_diff*exp(-n/2*post_diff)*1/sqrt(abs(h1/h2))
  return(result)
}
...
mcmc=function(n,s,v,lambda,tol,size,burnin,q){
  ...
  for(i in 1:(size+burnin)){
    if(i<=burnin){
      ...
      det_cand=det(hessian_cand)
      alpha=min(c(1,post_ratio(cov_cand,cov_temp,det_cand,det_temp,n,s,edge_cand,edge_temp,v,lambda,q)))
      u=runif(1,0,1)
